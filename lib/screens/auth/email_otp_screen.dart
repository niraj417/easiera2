...
class _EmailOTPScreenState extends State<EmailOTPScreen> {
  ...
  Future<void> _sendOTP() async {
    final email = widget.email.isNotEmpty ? widget.email : 'user@example.com';
    setState(() {_isSending = true; _errorMessage = null;});

    final result = await BrevoService.sendEmailOTP(email);
  ...
  Future<void> _verifyOTP() async {
    ...
    final email = widget.email.isNotEmpty ? widget.email : 'user@example.com';
    final isValid = BrevoService.verifyEmailOTP(email, enteredOtp);
  ...
  String get _maskedEmail {
    final e = widget.email.isNotEmpty ? widget.email : 'user@example.com';
    final parts = e.split('@');
    if (parts[0].length > 2) {
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
              ...
              child: Row(
                children: [
                  _isSending
                      ...
                      : Icon(
                          _otpSent ? Icons.email_outlined : Icons.error_outline,
                  ...
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(_isSending ? 'Sending OTP...' : (_otpSent ? 'OTP sent to' : 'Send failed'), style: AppTypography.bodySmall),
                        Text(_maskedEmail, style: AppTypography.labelLarge.copyWith(color: AppColors.primaryBlue)),
                      ],
                    ),
                  ),
            ...
            Text('Check your inbox & spam folder', style: AppTypography.bodySmall.copyWith(color: AppColors.statusAmber)),
            ...
            BHButton(label: 'Verify & Continue', onPressed: _verifyOTP, isLoading: _isLoading),
            const SizedBox(height: 24),
            Row(
...
class _OTPBox extends StatelessWidget {
  ...
  const _OTPBox({required this.controller, required this.focusNode, this.nextFocus, this.prevFocus, this.onFilled});
